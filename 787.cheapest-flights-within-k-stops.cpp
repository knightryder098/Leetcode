...
public:
    int findCheapestPrice(int n, vector<vector<int>> &flights, int src, int dst, int k)
    {
        ...
        while (!q.empty())
        {
            ...
            int curcost = it.second.second;
            if (stop > k)
                continue;
            for (auto x : graph[node])
            {
                int nextCity = x.first;
                int nextCityCost = x.second;

                if (curcost + nextCityCost < distance[nextCity] and stop <= k)
                {
                    distance[nextCity] = curcost + nextCityCost;
                    q.push({stop + 1, {nextCity, distance[nextCity]}});
                }
            }
        }
