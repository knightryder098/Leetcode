...
public:
    using ll = long long;
    int dfs(int src, map<int, vector<int>> &graph, vector<int> &visited)
    {
        visited[src] = 1;
        int tmp = 1;
    ...
    int maximumDetonation(vector<vector<int>> &bombs)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = i + 1; j < n; j++)
            {
                ...
                // check if i bomb can detonate j bomb
                //  if the distance between i and j is less than r1
                // then i can detonate j

                bool possible = ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) <= r1 * r1) ? true : false;
                if (possible)
                    graph[i].push_back(j);

                // check for j --> i
                possible = ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) <= r2 * r2) ? true : false;
                if (possible)
                    graph[j].push_back(i);
            }
        }
