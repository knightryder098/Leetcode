...
public:
    long long maxScore(vector<int>& nums1, vector<int>& nums2, int k) {
      vector<pair<int,int>>lola;
       int n=nums1.size();
       for(int i=0;i<n;i++){
        lola.push_back({nums1[i],nums2[i]});
       }
       priority_queue<int>pq;
       sort(begin(lola),end(lola),[&](auto &a,auto &b){return a.second>b.second;});
       long long sum=0,ans=0;
       for(int i=0;i<k;i++){
            sum+=lola[i].first;
            pq.push(-lola[i].first);
       }
       ans=sum*lola[k-1].second;
       for(int i=k;i<n;i++){
        sum+=lola[i].first+pq.top();
        pq.pop();
        pq.push(-lola[i].first);
        ans=max(ans,sum*lola[i].second);
       }
       return ans;
    }
};
