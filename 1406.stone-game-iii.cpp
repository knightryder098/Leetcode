...
class Solution {
public:
    int helper(int i,vector<int>&a,vector<int> &dp){
        int n=a.size();
        if (i ==n)return 0;
        if(dp[i]!=-1)return dp[i];
        int res = a[i] - helper(i + 1,a,dp);
        if (i + 2 <= n) {
            res = max(res, a[i] + a[i + 1] - helper(i + 2,a,dp));
        }
        if (i + 3 <= n) {
            res = max(res, a[i] + a[i + 1]
                + a[i + 2] - helper(i + 3,a,dp));
        }
        return dp[i]=res;
    }
    string stoneGameIII(vector<int>& stoneValue) {
       int n=stoneValue.size();
        vector<int>dp(n+1,-1);
       int res=helper(0,stoneValue,dp);
